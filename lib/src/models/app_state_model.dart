...
class AppStateModel extends Model {
  ...
  getLocalData() async {
    // print('api providerrrrrrrrrrrrrrrrrrrrrrrrrrrr');
    // print(apiProvider.toString());
  ...
  fetchAllBlocks() async {
   ...
   // print(" blocks isssssssssssssssssssss ${blocksString}");
    ...
    if (blocksString != null &&
        ...
        blocksString != '0') {
      try {
        //print('fetch blocks from providerrrrrrrrrrrrrrrrrrrrrrrrrrrrrr');
        ...
        print(" error in fetching the blocks $e");
      }
    }
  ...
  Future<bool> login(Map<String, dynamic> data, BuildContext context) async {
    ...
    if (response.statusCode == 200) {
      ...
      notifyListeners();
      /// will update fire base by log in  data
  ...
  Future<bool> register(data, BuildContext context) async {
    ...
    if (response.statusCode == 200) {
      user = Customer.fromJson(json.decode(response.body));
      print("${response.body}");
      print(' user phone noooooooooooooo ${user.billing.phone} ');
      //user.billing.phone
      if(user.id != null && user.id > 0) {
        _updatePushData();
        loggedIn = true;
  ...
  Future increaseQty(String key, int quantity) async {
    ...
    print(" shopping catttttttttttttttttttttttt ${formData.toString()}");

    if (response.statusCode == 200) {
      print('there are noooooooooooooooo error in post increasing qty');
      shoppingCart = CartModel.fromJson(json.decode(response.body));
     // print(" shopping catttttttttttttttttttttttt ${shoppingCart}");
      updateCartCount();
    } else {
      print('there are error in post increasing qty');
      return false;
     // throw Exception('Failed to load cart');
    }
    return true;
  ...
  Future<void> updateLanguage(String code) async {
   ...
   // apiProvider.filter['lan'] = code == null? 'en': code;
