...
class ProductDetail extends StatefulWidget {
  const ProductDetail({
    Key key,
    @required this.product, this.catName,
  }) : super(key: key);
...
class _ProductDetailState extends State<ProductDetail> {
  ...
  Widget build(BuildContext context) {
    print(' categorie name is inside produt details is ${widget.catName}');
    return ScopedModelDescendant<AppStateModel>(
        builder: (context, child, model) {
          print(model.blocks.pageLayout.product);
      if (model.blocks.pageLayout.product == 'layout1') {
        return ProductDetail1(product: widget.product,catName: widget.catName,);
