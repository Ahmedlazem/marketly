...
class _ProductsWidgetState extends State<ProductsWidget>
  ...
  void initState() {
    ...
    widget.productsBloc
        .fetchAllProducts(widget.productsBloc.productsFilter['id']);
  ...
  void _handleTabSelection() {
    if (widget.productsBloc.productsFilter['id'] !=
      ...
      widget.model.selectedRange =
          RangeValues(0, widget.model.blocks.maxPrice.toDouble());
      widget.productsBloc
          .fetchAllProducts(subCategories[_tabController.index].id.toString());
            ...
            ? PreferredSize(
                ...
                child: Align(
                  ...
                  child: TabBar(
                    ...
                    labelColor: Theme.of(context).primaryColorBrightness ==
                            Brightness.dark
                        ? Colors.white
                        : Colors.black,
                    indicator: MD2Indicator(
                        ...
                        indicatorColor:
                            Theme.of(context).primaryColorBrightness ==
                                ...
                                ? Colors.orangeAccent
                                : Colors.black,
                        //Theme.of(context).primaryColor,//Colors.white,//Theme.of(context).primaryColor,
        ...
        children: [
          ...
          StreamBuilder(
              ...
              builder: (context, AsyncSnapshot<List<Product>> snapshot) {
                print(' build stream  build products gird viewwwwwwwwwwwww');
                if (snapshot.hasData) {
                  if (snapshot.data.length != 0) {
                    //snapshot.data.first.vendor.
  ...
  /// vendor
  Widget buildVendorInfo({String vendorId, String vendorName, String vendorIcon}) {
    return
      Wrap(
        children: <Widget>[
          Column(
                      ...
                      children: <Widget>[
                        vendorIcon != null
                            ? CircleAvatar(
                                radius: 50,
                                backgroundImage:
                                    CachedNetworkImageProvider(vendorIcon),
  ...
  List<Widget> buildLisOfBlocks(AsyncSnapshot<List<Product>> snapshot) {
    ...
    print('categories name issssssssssssssss ${widget.name}');
    /// UnComment this if you use rounded corner category list in body.
    //list.add(buildSubcategories());
    if (snapshot.data != null) {
      list.add(ProductGrid(products: snapshot.data,catName: widget.name,));
        ...
        sliver: SliverList(
          delegate: SliverChildBuilderDelegate(
            (BuildContext context, int index) {
              return Column(
                children: <Widget>[
                  ProductItem(product: snapshot.data[index],category: widget.name,),
                  Divider(height: 0.0),
                ],
              );
            },
