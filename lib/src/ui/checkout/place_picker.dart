...
class PlacePickerState extends State<PlacePicker> {
  ...
  void getNearbyPlacesX(LatLng latLng) async {
    try {
      ...
      print('near places idddddddddddddddddddddddddddddd ${responseJson['results'][0]["place_id"]}');
      ...
      setState(() {
        ...
        print('place iddddddddddddddddddddddddddddddddddd $placeId');
  ...
  void initState() {
    ...
    markers.add(Marker(
      ...
      // position: widget.l  ?? LatLng(5.6037, 0.1870),
      position: widget.displayLocation ?? LatLng(5.6037, 0.1870),
      markerId: MarkerId("selected-location"),
    ));
    // print(markers);
    // print(' display location----------- ${widget.displayLocation}');
    // getNearbyPlacesX( latLng);
  }
        ...
        children: <Widget>[
          Expanded(
            child: GoogleMap(
              ...
              onTap: (latLng) {
                // print(' display location----------- ${widget.displayLocation.latitude}');
                clearOverlay();
  ...
  void decodeAndSelectPlace(String placeId) async {
    ...
    try {
      ...
      final responseJson = jsonDecode(response.body);
      print(
          'here get lat long from response json form google map ${responseJson}----------------------');

      if (responseJson['result'] == null) {
        print('here get lat long from apib ----------------------');
        throw Error();
      }
      ...
      print('here get lat long from apib ----------------------${location}');
    } catch (e) {
      print('here get lat long from apib ----------------------');
      print(e);
    }
  ...
  String getLocationName() {
    ...
    print(' city name is${this.locationResult.city}');
  ...
  await   getNearbyPlaces(latLng);
    print('gggggggggggggggggggggggggg');
    print(" lat lang is $latLng");
    try {
      //
      ...
      // "latlng=${latLng.latitude},${latLng.longitude}&" +w

      print(' ${response.body}');
      print(' get reverse geocode');
      if (response.statusCode != 200) {
        print(' in errorrrrrrrrrrr');
        throw Error();
      }

      final responseJson = jsonDecode(response.body);
      print("json responssssssssss ${responseJson['result']}");

      if (responseJson['result'] == null) {
        print(' in nullllllllll');
        throw Error().stackTrace;
      }

      final result = responseJson['result'];
      print('before set state nnnnnnnnnnnnnnnnnnnn ${result}');
      LocationResult();
      //print('${LocationResult().latLng}');
      // print('location reverse code iiiiiiiiiiiiiiiiiiiiiiiii  ${locationResult.city}');
      setState(() {
        if (result['address_components'] is List &&
            result['address_components'].length >= 15) {
          print('we aree hereeeeeeeeeeeeeeeeeeeeeeeeee');
          this.locationResult = LocationResult()
            ...
            ..country =
            AddressComponent.fromJson(result['address_components'][6])
          ...
          print(
              'we aree hereeeeeeeeeeeeeeeeeeeeeeeeee elseeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee*******');
          print('inside lov=cation ${result['formatted_address']}');
          this.locationResult = LocationResult()
            ..latLng = latLng
            ..formattedAddress = result['formatted_address']
            ..placeId = result['place_id']
            ...
            ..administrativeAreaLevel1 =
            AddressComponent.fromJson(result['address_components'][2])
            ..city = AddressComponent.fromJson(result['address_components'][1])
            ..country =
            AddressComponent.fromJson(result['address_components'][2]);
        }
      });
      ...
      print(' in errorfbbbbbbbbbbbbbbbbbbb');
      print(e);
    }
    //  setState(() {
    //    print('innnnnnnnnnnnnnnnnnnnn set state nnnnnnnnnnnnnnnnnnnn');
    //   // print(  AddressComponent.fromJson(result['address_components'][6]));
    //    locationResult = LocationResult()
    //      ..name = result['address_components'][0]['short_name']
    //      ..locality = result['address_components'][1]['short_name']
    //      ..latLng = latLng
    ...
    //    //  ..placeId = result['place_id']
    //    //  ..postalCode = result['address_components'][7]['short_name']
    //      ..country = AddressComponent.fromJson(result['address_components'][3])
    ...
    //   // this.locationResult = LocationResult();
    //  });
    print('location reverse code iiiiiiiiiiiiiiiiiiiiiiiii  $locationResult }');
    // } catch (e) {
    //   print(' in errrrrrrrrrrrrrrrrrrr resulthhhhhhhhhhhhhhhhhhhhhhhhhhh');
    //   print(e);
    // }
  }
  ...
  void moveToCurrentUserLocation() {
    ...
    Location().getLocation().then((locationData) {
      print(' start get location from location ${locationData.longitude}');
      LatLng target = LatLng(locationData.latitude, locationData.longitude);
      moveToLocation(target);
    }).catchError((error) {
      print('here error of location');
...
//
//       final responseJson = jsonDecode(response.body);
//       print('here get lat long from response json form google map ${responseJson}----------------------');
//
//       if (responseJson['result'] == null) {
//         print('here get lat long from apib ----------------------');
//         throw Error();
...
//       print('before set state nnnnnnnnnnnnnnnnnnnn');
//       LocationResult();
//       print('${LocationResult().latLng}');
//      // print('location reverse code iiiiiiiiiiiiiiiiiiiiiiiii  ${locationResult.city}');
//       setState(() {
