...
class ApiProvider {
  ...
  factory ApiProvider() {
  //  print( 'start api provider where ${_apiProvider.}');
    return _apiProvider;
  }
  ...
  Future<http.Response> fetchBlocks() async {
   ...
   // print(' cookies is $cookies');
    ...
    final response = await http.post(
      Uri.parse(config.url + '/wp-admin/admin-ajax.php?action=mstore_flutter-keys'  + '&lang=' + filter['lan'] ),

      headers: headers,
      body: filter,

    );
 // print(" this is headerrrrrrrrr ${config.url + '/wp-admin/admin-ajax.php?action=mstore_flutter-products' + '&lang=' + filter['lan'] + '&flutter_app=' + '1'}");
    print(" this is headerrrrrrrrr ${config.url + '/wp-admin/admin-ajax.php?action=mstore_flutter-keys'  + '&lang=' + filter['lan']  }");
    //print("${response.body}");
  ...
  Future<List<Product>> fetchProductList(Map data) async {
    ...
    headers['content-type'] =
    'application/x-www-form-urlencoded; charset=utf-8';
  print(' headerrrrrrrrrrrrrrrrr is ${headers}');
  print(' dataaaaaaaaaaaaaaaaaaaaaaaaaaaaa ${data}');
   // print( " which post on i it isssssssssssssssssssss ${config.url + '/wp-admin/admin-ajax.php?action=mstore_flutter-products' + '&lang=' + filter['lan'] + '&flutter_app=' + '2'}");
    final response = await http.post(
      Uri.parse(config.url + '/wp-admin/admin-ajax.php?action=mstore_flutter-products' + '&lang=' + filter['lan'] + '&flutter_app=' + '1'),
      headers: headers,
      body: data,
    );
    if (response.statusCode == 200) {
      print(" responseeeeeeeeeeeeeeeeeeeeeeeeeee is${response.body}");
  ...
  Future<dynamic> post(String endPoint, Map data) async {
    print(' post fun in API provider end point is ${endPoint} and data is ${data}');
    data.addAll(filter);
    headers['content-type'] =
    'application/x-www-form-urlencoded; charset=utf-8';
    final response = await http.post(
